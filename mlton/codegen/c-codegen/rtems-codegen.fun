...
functor RtemsCodegen (S: RTEMS_CODEGEN_STRUCTS): RTEMS_CODEGEN =
struct
...
val traceGotoLabel = Trace.trace ("RtemsCodegen.gotoLabel", Label.layout, Unit.layout)
   ...
   let
      ...
      fun declareMain () =
         if !Control.emitMain andalso !Control.format = Control.Executable
            then 
                 let
                    val rtemsCfg = SOME (Pervasive.TextIO.openIn "/tmp/rtemsMacros.cfg") handle _ => NONE
                    fun outputAll stream = print (Pervasive.TextIO.inputAll stream) before Pervasive.TextIO.closeIn stream
                    fun doit (SOME stream) = outputAll stream
                      | doit NONE = ()
                 in
                    print RtemsMain.defaultMain;
                    doit rtemsCfg;
                    print "#include <rtems/confdefs.h>"
                 end
...
fun output {program as Machine.Program.T {chunks,
   ...
   let
      ...
      val {get = labelInfo: Label.t -> {block: Block.t,
           ...
           set = setLabelInfo, ...} =
         Property.getSetOnce
         (Label.plist, Property.initRaise ("RtemsCodegen.info", Label.layout))
                           ...
                           val prim =
                              ...
                              in
                                 case Prim.name prim of
                                  ...
                                  | _ => Error.bug "RtemsCodegen.outputTransfer: Arith"
                              end
                               ...
                               | Indirect =>
                                    let
                                       val (fptr,args) =
                                          case args of
                                             (fptr::args) => (fptr, args)
                                           | _ => Error.bug "RtemsCodegen.outputTransfer: CCall,Indirect"
                                       val name =
                                          concat ["(*(",
                           ...
                           fun doit {cases: (string * Label.t) vector,
                              ...
                              in
                                 case (Vector.length cases, default) of
                                    (0, NONE) =>
                                       Error.bug "RtemsCodegen.outputTransfers: Switch"
